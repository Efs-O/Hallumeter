// Forge AI (VS Code) — reads GitHub Copilot CLI session events. Forge uses @github/copilot-sdk
// with local Copilot CLI; session layout matches ~/.copilot/session-state (see GitHub Copilot CLI docs).
...
fn forge_session_state_root() -> Option<PathBuf> {
...
fn collect_forge_session_dirs(session_state: &Path) -> Vec<(SystemTime, PathBuf)> {
...
fn forge_normalize_model_id(raw: &str) -> String {
...
fn parse_events_jsonl(content: &str) -> Option<(String, f64, u64)> {
    let mut scan = ForgeScan::default();
    ...
    let (tokens, limit_u64, model_raw) =
        if let (Some(ct), Some(lim)) = (scan.current_tokens, scan.token_limit) {
            ...
            let normalized = forge_normalize_model_id(&m_raw);
            let lim = curves
                .models
                .iter()
    ...
    let normalized = forge_normalize_model_id(&model_raw);
...
/// Active Forge / Copilot SDK sessions under Copilot CLI session-state.
/// Returns `(model, fill_pct, session, tokens, last_active_ms)`.
pub fn read_forge_usage(
    activity_secs: u64,
    max_files: usize,
) -> Option<(String, f64, String, u64, i64)> {
    let session_state = forge_session_state_root()?;
    if !session_state.is_dir() {
        return None;
    }
    let cutoff = recent_cutoff(activity_secs);

    collect_forge_session_dirs(&session_state)
        .into_iter()
        .take(max_files)
...
{"type":"session.usage_info","timestamp":"2026-04-01T12:01:00.000Z","id":"b","parentId":"a","ephemeral":true,"data":{"currentTokens":50000,"tokenLimit":128000,"messagesLength":4}}"#;
    ...
    fn normalizes_deploy_name_dots() {
        assert_eq!(
            forge_normalize_model_id("my.gpt-4.1-deployment"),
